...
SETTINGS = ['sharpness', 'contrast', 'brightness', 'saturation', 'ISO', 'video_stabilization', 'exposure_compensation',
            'exposure_mode', 'meter_mode', 'awb_mode', 'image_effect', 'color_effects', 'rotation', 'hflip', 'vflip', 'crop']
...
class Camera_RPi(picamera.PiCamera):
    ...
    def show_settings(self):
        for attr in SETTINGS:
            print('{} {} : {}'.format(SETTINGS.index(
                attr), attr, getattr(self, attr)))
    ...
    def preview(self):
        ...
        try:
            while True:
                ...
                setattr(self, SETTINGS[int(in_command)], type(
                    getattr(self, SETTINGS[int(in_command)]))(in_vaule))
    ...
    def custom_capture(self, num=3, delay=0):
        filenames = [os.path.join(IMAGE_PATH, 'image%02d' % i + datetime.datetime.now(
        ).strftime('(%d-%m-%y-%H:%M:%S)') + '.jpg') for i in range(num)]
        print('Capturing...')
